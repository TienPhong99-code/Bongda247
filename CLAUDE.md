...
- **Comment** (SP2.2): hook `comment_post` → cộng 5đ comment đầu/bài (user đăng nhập, `$approved===1`). `comments.php` render list+form (native WP, KHÔNG plugin). Cần 2 WP option: `comment_registration=1` (bắt đăng nhập) + **`comment_previously_approved=0`** (tự duyệt cả comment đầu → cộng điểm ngay, KHÔNG giữ chờ duyệt). WP flood-control tự chặn spam comment liên tiếp.
- **Reply lồng (threaded):** dùng threaded comment gốc WP (option `thread_comments=1` depth 5). `bd_comment_render` xuất `comment_reply_link` (chỉ user đăng nhập); `functions.php` enqueue script `comment-reply` (khi `is_singular+comments_open+thread_comments`); CSS thụt lề `#comments .children`. Reply = comment cùng bài → **KHÔNG cộng điểm lại** (dedup per-post `bd_comment_posts`).
- **Like bình luận:** nút tim + số trên mỗi comment. AJAX `bd_toggle_comment_like` (nonce `bd_points` + login, KHÔNG nopriv). Comment meta `bd_comment_likes` (int đếm) + user meta `bd_liked_comments` (mảng comment ID đã thích). **Chỉ tín hiệu xã hội, KHÔNG cộng điểm** (comment/like không giới hạn → tránh farm). Khách: chỉ hiện số đếm tĩnh (nếu >0). *(Lưu ý: khi đọc `bd_liked_comments` phải cast `(int) $comment->comment_ID` vì in_array strict — comment_ID là string.)*
